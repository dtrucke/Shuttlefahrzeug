...
protected:
    ...
    {
        // Define the region to crop (top-right quarter)
        int width = image_orig.cols;
        int height = image_orig.rows;
        Rect roi( 0, 0, width, height / 2 );
        Mat croppedImage = image_orig( roi ).clone();

        // Convert the cropped image to HSV
        Mat img_hsv;
        cvtColor( croppedImage, img_hsv, COLOR_BGR2HSV );

        // Define color ranges for traffic lights (red, yellow, green)
        Scalar lower_red1( 0, 100, 100 ), upper_red1( 10, 255, 255 );
        Scalar lower_red2( 160, 100, 100 ), upper_red2( 180, 255, 255 );
        Scalar lower_yellow( 20, 100, 100 ), upper_yellow( 30, 255, 255 );
        Scalar lower_green( 40, 100, 100 ), upper_green( 90, 255, 255 );

        // Threshold images
        Mat mask_red, mask_yellow, mask_green;
        inRange( img_hsv, lower_red1, upper_red1, mask_red );
        Mat mask_red2;
        inRange( img_hsv, lower_red2, upper_red2, mask_red2 );
        mask_red = mask_red | mask_red2;
        inRange( img_hsv, lower_yellow, upper_yellow, mask_yellow );
        inRange( img_hsv, lower_green, upper_green, mask_green );

        // Combine all masks
        Mat mask = mask_red | mask_yellow | mask_green;
        ...
        // Draw bounding box if a traffic light is detected
        if( maxArea > 20 )  // Threshold to remove noise
        {
            rectangle( croppedImage, boundingBox, Scalar( 255, 0, 0 ), 2 );
        ...
        // Display result
        imshow( "Traffic Light Detection", croppedImage );
...
protected:
    ...
    bool processBox( tronis::BoxDataSub* msg )
    {
        ...
        for( int i = 0; i < msg->Objects.size(); i++ )
        {
            ...
            string actor_name = box.ActorName.Value();
            if( actor_name == "SnappyRoad" || actor_name == "SkySphere" ||
                actor_name == "TronisTrack" )
            {
                continue;
            }
            if( traffic_light_detected )
            {
                if( box.Pose.Location.Y > 250 )
                {
